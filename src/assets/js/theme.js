...
var getColor = function getColor(name) {
  ...
  return getComputedStyle(dom).getPropertyValue("--slick-".concat(name)).trim();
};
...
var tableCollationInit = function tableCollationInit() {
  var tableCollations = document.querySelectorAll("[data-table-collation]");
  tableCollations.forEach(function (table) {
    var rows = table.querySelectorAll("thead > tr, tbody > tr, tfoot > tr");
    var numCols = table.querySelectorAll("thead > tr > th").length;
    var col_i = 1;
    ...
    var _loop = function _loop() {
      ...
      rows.forEach(function (row, index) {
        ...
        var first = row.querySelector("td,th").cloneNode(true);
        first.classList.add("st-key");
        var target = col_i;
        var second = row.querySelectorAll("td,th")[target].cloneNode(true);
...
var dropdownOnHover = function dropdownOnHover() {
  var navbarArea = document.querySelectorAll('[data-sl-dropdown-on-hover]');

  if (navbarArea) {
    navbarArea.forEach(function (navbarItem) {
      navbarItem.addEventListener('mouseover', function (e) {
        if (e.target.tagName !== 'path' && e.target.tagName !== 'svg') {
          var _e$target$className;

          if ((_e$target$className = e.target.className) !== null && _e$target$className !== void 0 && _e$target$className.includes('dropdown-toggle')) {
